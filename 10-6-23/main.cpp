...
void determineOutcome(std::vector<Player> &players) {
    ...
    for (Player& player : players) {
        // Skip players who did not draw a card
        std::cout << player.drawnCard;
    ...
    if (winner != nullptr) {
        std::cout << "Player " << winner->getUsername() << " wins this round with ";
...
void playOneRound(int numberOfPlayers, std::vector<Player> &players, Deck& deck)
{
    for (int i = 0; i < numberOfPlayers; i++)
    {
        ...
        if (i == 0) {
            ...
            std::cout << "You have 15 seconds to to draw a card. Enter a number (1-9)" << std::endl;
